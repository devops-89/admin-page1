...
// helpers (same as above; paste or import)
const safeParse = (v) => {
  if (!v) return {};
...
//   od?.Airport?.AirportCode || od?.Airport?.CityCode || "-";
...
const FlightTabMultiCity = ({ onView }) => {
  const columns = useMemo(
    () => [
      //   { key: "from", label: "From" },
      //   { key: "to", label: "To" },
      //   { key: "segments", label: "Segments" },
      //   { key: "first_departure", label: "First Departure" },
      //   { key: "last_arrival", label: "Last Arrival" },
  ...
  const table_heading = useMemo(
    () => ({
      heading: "Multi-city bookings",
      para: rows.length
        ? "Manage complex itineraries with multiple legs."
        : "No Data Found",
    }),
    [rows.length]
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const docs =
          res?.data?.docs ?? res?.docs ?? (Array.isArray(res) ? res : []);
        const filtered = docs.filter(
          (d) => normalizeJourneyType(d?.journey_type) === "MULTICITY"
        );
...
export default FlightTabMultiCity;
